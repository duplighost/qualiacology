...
export class Player {
  constructor(camera, world, dom) {
    ...
    // mobile analog input, driven by the touch handlers in main.js
    this.moveX = 0; this.moveY = 0;    // left stick, -1..1
    this.lookDX = 0; this.lookDY = 0;  // right stick, pixel deltas accrued this frame
    this.touchActive = false;          // a touch device is driving input
    this.run = false;
  ...
  update(dt) {
    ...
    const k = this.keys;
    let fwd = 0, str = 0;
    if (!this.frozen && (this.locked || this.touchActive)) {
      if (k['KeyW'] || k['ArrowUp']) fwd += 1;
      if (k['KeyS'] || k['ArrowDown']) fwd -= 1;
      if (k['KeyA'] || k['ArrowLeft']) str -= 1;
      if (k['KeyD'] || k['ArrowRight']) str += 1;
      // mobile: left stick to move, right stick already fed into lookD*
      fwd += -this.moveY; str += this.moveX;
      this.yaw -= this.lookDX * this.sens;
      this.pitch -= this.lookDY * this.sens;
      this.pitch = Math.max(-1.45, Math.min(1.45, this.pitch));
    }
    this.lookDX = 0; this.lookDY = 0;
    const running = ((k['ShiftLeft'] || k['ShiftRight']) || this.run) && fwd > 0;
