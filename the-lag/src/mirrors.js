...
export class Mirrors {
  ...
  update(scene, camera) {
    ...
    for (const m of this.mirrors) {
      m.setActive(false);
      // Unbind last frame's reflection texture before rendering into the pooled
      // targets again. Merely branching around the sample in the shader still
      // leaves that target bound as an input texture, which WebGL correctly
      // rejects as a framebuffer feedback loop.
