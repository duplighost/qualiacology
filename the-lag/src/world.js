...
export class World {
  ...
  _exposeDebug() {
    window.HOM = {
      ...
      shot: async (name = 'the-lag') => {
        // Developer-only capture helper. The original build POSTed to a local
        // /__save test endpoint that does not exist on Netlify. Download the
        // PNG in the browser instead, so invoking HOM.shot() cannot create a
        // production 404 or depend on a private development server.
        this._render();
        const canvas = this.renderer.domElement;
        const blob = await new Promise((resolve) => canvas.toBlob(resolve, 'image/png'));
        if (!blob) throw new Error('Screenshot capture failed');
        const safe = String(name).replace(/[^a-z0-9_-]+/gi, '-').replace(/^-+|-+$/g, '') || 'the-lag';
