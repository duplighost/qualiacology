...
Copied from WINTERLINE with their credits above; every image was re-encoded at 1024 px or less (`tools/blender/gun_glb_1k.py`); geometry, skins and animation are unchanged.

## Hunting rifle - Poly Haven, CC0
...
`range/*.glb` are Poly Haven models under **CC0 1.0**: Wooden Crate 02, Cardboard Box 01, Street Lamp 01, Barrel 01, Wooden Picnic Table, Lantern 01 (https://polyhaven.com/models), packed into GLBs with 1K images by `tools/blender/gun_pack.py`. `range/ground.*` and `range/gravel.*` are ambientCG **Ground048** and **Gravel022** (CC0 1.0) encoded by `tools/textures/encode.py`.
