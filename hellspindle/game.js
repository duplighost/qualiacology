(() => {
  ...
  const H = 900;
  let renderScale = 1;
  // Resolve against this script, not the embedding page (/tests/, subfolders,
  // or another site's shell). The offline edition supplies identical Blob bytes.
  const scriptURL = document.currentScript && document.currentScript.src;
  const assetBase = scriptURL ? new URL('.', scriptURL).href : '';
  const assetURL = path => (window.__HELLSPINDLE_ASSETS__ && window.__HELLSPINDLE_ASSETS__[path]) ||
    (assetBase ? new URL(path, assetBase).href : path);
  const BG_PATHS = [
    'assets/bg_nave.webp',
  ...
  function loadAnim(folder, count) {
    ...
    for (let i = 0; i < count; i++) {
      const img = new Image();
      img.decoding = 'async';
      img.src = assetURL(`assets/sprites/anim/${folder}/${String(i).padStart(2, '0')}.png`);
      frames.push(img);
    }
    return frames;
  }
  ...
  function sizeBackingStore() {
    if (COARSE_POINTER && window.innerHeight > window.innerWidth &&
        (game.state === 'playing' || game.state === 'dead') && !game.paused) setPaused(true);
  ...
  const TOUCH_PAUSE = { x: W - 104, y: 26, w: 80, h: 80 };
  const TITLE_NEW = { x: W / 2 - 180, y: 758, w: 360, h: 58 };
  const PAUSE_ACTIONS = [
    { key: 'resume', x: 450, y: 548, w: 340, h: 62 },
    { key: 'relics', x: 810, y: 548, w: 340, h: 62 },
    { key: 'checkpoint', x: 450, y: 628, w: 340, h: 62 },
    { key: 'mute', x: 810, y: 628, w: 340, h: 62 },
    { key: 'fullscreen', x: 630, y: 708, w: 340, h: 62 }
  ];
  const containsPoint = (r, p) => p.x >= r.x && p.x <= r.x + r.w && p.y >= r.y && p.y <= r.y + r.h;
  ...
  function toggleFullscreen() {
    ...
    } catch (_) { /* Optional/denied fullscreen must never interrupt play. */ }
  }
  ...
  function setRelicsOpen(open) {
    game.relicsOpen = !!open;
    game.relicNudge = 0;
    input.reset();
    if (open) saveGame();
    audio.tone(open ? 320 : 180, 0.1, 'triangle', 0.05, open ? 60 : -60);
  }

  function handleInterfacePointer(p) {
    if (game.state === 'title') {
      if (hasSave() && containsPoint(TITLE_NEW, p)) restartFullRun();
      else startGame();
      input.reset();
      return true;
    }
    if (game.state === 'victory') {
      if (game.victoryTimer > 0.8) restartFullRun();
      input.reset();
      return true;
    }
    if (game.paused) {
      const action = PAUSE_ACTIONS.find(r => containsPoint(r, p));
      if (action) {
        if (action.key === 'resume') setPaused(false);
        else if (action.key === 'relics') { setPaused(false); setRelicsOpen(true); }
        else if (action.key === 'checkpoint') restartFromCheckpoint();
        else if (action.key === 'mute') audio.toggleMute();
        else if (action.key === 'fullscreen') toggleFullscreen();
      }
      input.reset();
      return true;
    }
    ...
    if ((COARSE_POINTER || input.lastInputWasTouch) && containsPoint(TOUCH_PAUSE, p)) {
      setPaused(true);
      return true;
    }
    if (player.relicPoints > 0 && containsPoint(RELIC_BADGE, p)) {
      setRelicsOpen(true);
      return true;
    }
    return false;
  ...
  class Input {
    constructor(target) {
      ...
      window.addEventListener('keydown', e => {
        const code = e.code;
        if (!this.keys.has(code) && !e.repeat) {
          this.pressed.add(code);
          if (code !== 'KeyM' && code !== 'KeyF') this.anyPressed = true;
          if (['Space', 'KeyW', 'ArrowUp'].includes(code) && game.state === 'playing' && !game.paused && !game.relicsOpen) this.jumpQueued = true;
          if (['Escape', 'KeyP'].includes(code)) this.pauseQueued = true;
          // Fullscreen requires a real user gesture, not a later animation tick.
          if (code === 'KeyF') { toggleFullscreen(); this.pressed.delete(code); }
        }
        this.keys.add(code);
      ...
      target.addEventListener('pointerdown', e => {
        ...
        if (handleInterfacePointer(p)) return;
      ...
      target.addEventListener('lostpointercapture', release, { passive: false });
      const suspend = () => {
        this.reset();
        if (game.state === 'playing' || game.state === 'dead') setPaused(true);
        if (audio.ctx && audio.ctx.state === 'running') audio.ctx.suspend().catch(() => {});
      };
      window.addEventListener('blur', suspend);
      document.addEventListener('visibilitychange', () => { if (document.hidden) suspend(); });
      window.addEventListener('pagehide', () => { if (game.state !== 'title') saveGame(); });
    }
    ...
    pollGamepad() {
      let pad = null;
      try { pad = Array.from(navigator.getGamepads ? navigator.getGamepads() : []).find(Boolean) || null; }
      catch (_) { /* Permissions Policy may disable gamepads in an iframe. */ }
      this.pad = pad;
      if (!pad) { this.padGone = true; this.gamepadJumpPrev = false; this.gamepadPausePrev = false; return; }
      const jump = !!(pad.buttons[0] && pad.buttons[0].pressed);
      // Chrome stops reporting pads to an unfocused page. When one comes back
      // with a button still physically held, that is not a new press: take the
      // reading as the baseline instead of firing an edge off it.
      if (this.padGone) {
        this.padGone = false;
        this.gamepadJumpPrev = jump;
        this.gamepadPausePrev = !!(pad.buttons[9] && pad.buttons[9].pressed);
        return;
      }
      if (jump && !this.gamepadJumpPrev) {
        if (game.state === 'playing' && !game.paused && !game.relicsOpen) this.jumpQueued = true;
        this.anyPressed = true;
        if (game.state === 'victory' && game.victoryTimer > 0.8) this.pressed.add('KeyR');
  ...
  function deathFrame(frames, e, seconds) {
    ...
    return animOnce(frames, t, frames.length / Math.max(0.001, seconds));
  }
  ...
  function gainXP(amount) {
    if (player.dead || !Number.isFinite(amount) || amount <= 0) return;
  ...
  function loadGame() {
    try {
      ...
      const integer = (v, fallback, lo, hi) => Math.floor(finite(v, fallback, lo, hi));
      player.level = integer(s.level, 1, 1, 50);
      player.xp = finite(s.xp, 0, 0, xpToNext(50));
      for (const def of RELICS) player.relics[def.key] = integer(s.relics && s.relics[def.key], 0, 0, def.max);
      const spent = RELICS.reduce((n, def) => n + player.relics[def.key], 0);
      player.relicPoints = integer(s.relicPoints, Math.max(0, player.level - 1 - spent), 0, 49);
      applyPower();

      let x = finite(s.checkpointX, 250, 25, WORLD_W - 25);
      let y = finite(s.checkpointY, 760, PLAYER_H, 1080);
      const r = playerRectAt(x, y);
      const supported = platforms.some(p => Math.abs(y - p.y) < 2 && r.x >= p.x && r.x + r.w <= p.x + p.w);
      const embedded = platforms.some(p => !p.oneWay && rectsOverlap(r, p));
      if (!supported || embedded || checkpointBlocked(x, y)) {
        const area = areaIndexAt(x);
        const cp = area === 0 ? { x: 250, y: 760 } : AREA_CHECKPOINTS[area];
        x = cp.x; y = cp.y;
      }
      player.checkpointX = x;
      player.checkpointY = y;
      player.visited = Array.isArray(s.visited)
        ? [...new Set(s.visited.filter(v => Number.isInteger(v) && v >= 0 && v < AREAS.length))] : [0];
      if (!player.visited.includes(0)) player.visited.unshift(0);
      if (!player.visited.includes(areaIndexAt(x))) player.visited.push(areaIndexAt(x));
      game.kills = integer(s.kills, 0, 0, 1000000000);
      game.maxCombo = integer(s.maxCombo, 0, 0, 1000000000);
      for (let i = 0; i < seals.length; i++) {
        const bandHp = Array.isArray(s.seals) && Array.isArray(s.seals[i]) ? s.seals[i] : [];
        for (let b = 0; b < seals[i].bands.length; b++) {
          const band = seals[i].bands[b];
          band.hp = finite(bandHp[b], band.maxHp, 0, band.maxHp);
          band.flash = 0;
        }
        seals[i].alive = seals[i].bands.some(band => band.hp > 0);
        seals[i].breached = membranePassable(seals[i]);
      }
      for (let i = 0; i < caches.length; i++) caches[i].taken = !!(Array.isArray(s.caches) && (s.caches[i] === 1 || s.caches[i] === true));
      game.zone = areaIndexAt(x);
      game.zoneTitle = AREAS[game.zone].name;
      game.completed = s.completed === true;
  ...
  function startGame() {
    ...
    if (hasSave()) {
      if (loadGame()) restartFromCheckpoint();
      else restartFullRun();
    }
    if (game.completed) {
      game.state = 'victory';
  ...
  function restartFromCheckpoint() {
    ...
    // frame. Same for a pause tapped while dead. Only the queued/discrete
    // state is cleared: a thumb still on the glass, or a key still held, is
    // the player still steering, and the respawn must not take it away.
    input.pressed.clear();
    input.leftJumpLatch = false;
    ...
    game.shakeX = 0;
    game.shakeY = 0;
    const respawnZone = areaIndexAt(player.checkpointX);
    // Name the district only when the respawn actually moved you to another
    // one. Dying twice in the same room should not reintroduce the card.
    if (respawnZone !== game.zone) game.zoneTitleTimer = 3.2;
    game.zone = respawnZone;
    game.zoneTitle = AREAS[game.zone].name;
    game.deathTimer = 0;
    game.bossDeadTimer = 0;
    game.camera.look = 0;
    ...
    resetEnemies();
    syncBossState();
    if (game.completed) { game.state = 'victory'; game.victoryTimer = 1.2; }
  ...
  function killPlayer() {
    if (player.dead || game.state !== 'playing' || game.completed) return;
    player.dead = true;
    player.health = 0;
    player.vx = 0;
    player.vy = -240;
    ...
    player.deathFloor = corpseRestY(player.x, player.y);
    saveGame();
    player.deathTime = 0;
    game.state = 'dead';
    game.deathTimer = 2.1;
  ...
  function moveGroundEnemy(e, dt) {
    ...
    if (e.y > 1120) {
      if (e.type === 'boss') {
        // A fall must never bypass killEnemy and strand the ending. The arena
        // has real gaps: recover to his authored footing without restoring HP.
        e.x = e.baseX; e.y = e.baseY;
        e.vx = 0; e.vy = 0; e.grounded = true;
        e.state = 'idle'; e.stateT = 0; e.cooldown = 0.65; e.attackHit = false;
        smokePuff(e.x, e.y - 30, 8, '#3b0b18');
      } else e.alive = false;
    }
  }
  ...
  function applyGrappleConstraint(dt) {
    ...
    if (dist > rope) {
      ...
      // Even when masonry blocks this correction, retain the attachment.
      // A temporary stretch is preferable to an unsolicited release.
    }
  ...
  function releaseHook(boost = false) {
    ...
    if (boost) {
      ...
      const direction = sign0(tangential) || sign0(input.moveX() * tx) || sign0(player.facing * tx) || 1;
  ...
  function updatePlayer(dt) {
    ...
    if (!yoyo.latched) {
      ...
      if (Math.abs(move) > 0.08) {
        // Do not erase a rope launch or wall-jump by clamping it to walk speed.
        // Steering may brake excess momentum, but cannot accelerate it further.
        if (Math.abs(player.vx) <= maxSpeed) {
          player.vx = clamp(player.vx + move * accel * dt, -maxSpeed, maxSpeed);
        } else if (sign0(move) !== sign0(player.vx)) {
          player.vx = approach(player.vx, move * maxSpeed, accel * dt);
        } else if (player.grounded) {
          player.vx = approach(player.vx, move * maxSpeed, accel * 0.5 * dt);
        }
        player.facing = move > 0 ? 1 : -1;
    ...
    if (player.y > 1080) { killPlayer(); return; }
  ...
  function damagePlayer(amount, knockX = 0, knockY = -180) {
    if (player.invuln > 0 || player.dead || game.state !== 'playing' || game.completed) return;
  ...
  function updateExecutioner(e, dt) {
    ...
    if (e.state === 'windup') {
      ...
      if (!e.attackHit && e.stateT < 0.20) {
        ...
        if (rectsOverlap(playerRect(), { x: e.x - 110, y: e.y - 120, w: 220, h: 120 })) damagePlayer(22, sign0(player.x - e.x), -340);
      }
  ...
  function updateBoss(e, dt) {
    ...
    if (e.state === 'slam') {
      ...
      if (!e.attackHit && e.stateT < (e.phase === 2 ? 0.38 : 0.5)) {
        ...
        hitStop(0.06, 0.13);
        audio.hit(true);
        if (rectsOverlap(playerRect(), { x: e.x - 150, y: e.y - 150, w: 300, h: 150 })) damagePlayer(30, sign0(player.x - e.x), -430);
      }
      if (e.stateT <= 0) {
        e.state = 'idle';
    ...
    if (e.grounded && e.vy >= 0 && Math.abs(e.vx) > 1 && !hasGroundAhead(e, sign0(e.vx))) {
      e.vx = 0;
      if (e.state === 'charge') { e.state = 'idle'; e.cooldown = 0.35; }
    }
    moveGroundEnemy(e, dt);
  }
  ...
  function updateEnemies(dt) {
    for (let i = enemies.length - 1; i >= 0; i--) {
      ...
      if (!e.alive) {
        ...
        if (e.restY === undefined) e.restY = corpseRestY(e.x, e.y);
        if (e.restY == null || e.y < e.restY) {
          e.vy = (e.vy || 0) + 2100 * dt;
          e.y += e.vy * dt;
          if (e.restY != null && e.y >= e.restY) { e.y = e.restY; e.vy = 0; }
        }
  ...
  function killEnemy(e, hitX, hitY, knockX, knockY) {
    if (!e.alive) return;
    if (player.relics.carrion > 0 && !player.dead) {
    ...
    if (!player.dead) player.health = Math.min(player.maxHealth, player.health + (e.type === 'boss' ? 0 : 2 + Math.min(5, player.killStreak * 0.4)));
  ...
  function handleYoyoCombat(dt) {
    ...
    for (const e of enemies) {
      ...
      if (chainHot && e.chainCooldown <= 0 && lineHitsRect(pc.x, pc.y, yoyo.x, yoyo.y, er, 3)) {
        ...
        damageEnemy(e, chainDamage, clamp(e.x, Math.min(pc.x, yoyo.x), Math.max(pc.x, yoyo.x)), e.y - e.h * 0.5, side * 18, -8, yoyo.charge > 0.9);
  ...
  function updateProjectiles(dt) {
    for (let i = projectiles.length - 1; i >= 0; i--) {
      ...
      const wave = p.type === 'wave' || p.type === 'bigWave';
      const solids = getDynamicSolids(p.x, 220);
      const support = wave ? solids.find(s => !s.membrane && !s.bossGate && p.x >= s.x && p.x <= s.x + s.w && Math.abs(s.y - (p.y + 13)) < 8) : null;
      const blocked = (wave && !support) || solids.some(s => !s.oneWay && s !== support && lineHitsRect(oldX, oldY, p.x, p.y, s, p.r));
      if (blocked) {
        if (Math.abs(p.x - player.x) < W) sparkBurst(p.x, p.y, 4, 180, '#ff8a72');
        projectiles.splice(i, 1);
        continue;
      }
      ...
      if (p.hostile && !p.reflected && circleRect(yoyo.x, yoyo.y, yoyo.r + 7, { x: p.x - p.r, y: p.y - p.r, w: p.r * 2, h: p.r * 2 })) {
        ...
        let dx = p.x - yoyo.x;
        let dy = p.y - yoyo.y;
        if (hypot(dx, dy) < 0.001) { dx = -p.vx || player.facing; dy = -p.vy; }
        const len = hypot(dx, dy) || 1;
      ...
      if (p.y > 1180 || p.y < -400 || p.x < -300 || p.x > WORLD_W + 300) projectiles.splice(i, 1);
    }
  }
  ...
  function updateGame(dt) {
    ...
    if (input.consumePause() && game.state !== 'title' && game.state !== 'victory') {
      if (game.relicsOpen) setRelicsOpen(false);
    ...
    if (game.state === 'victory') {
      ...
      updateParticles(dt);
      updateCamera(dt);
      // Pointer restart is handled in the pointer event; R is handled above.
      // Other keys (especially M/F) must not destroy the completed save.
      input.consumeAny();
      return;
    }

    if (game.completed) {
      // The finishing blow commits the win. Keep the corpse/particles moving,
      // but do not allow a surviving bat or a pit to cancel the ending.
      updateEnemies(dt);
      updateParticles(dt);
      updateCamera(dt);
      audio.update(yoyo, 'victory');
      game.bossDeadTimer = Math.max(0, game.bossDeadTimer - dt);
      if (game.bossDeadTimer <= 0) {
        game.state = 'victory';
        game.victoryTimer = 0;
        // The one victory sound. The old tail below this branch can no longer
        // be reached, so the sting has to fire here or it never fires at all.
    ...
    updateYoyo(dt);
    updatePlayer(dt);
    if (player.dead) { audio.update(yoyo, 'dead'); return; }
    updateEnemies(dt);
    if (player.dead) { audio.update(yoyo, 'dead'); return; }
    handleYoyoCombat(dt);
  ...
  function drawKnight(g, e) {
    ...
    if (drawGroundSprite(g, 'knight', 0, 0, 136, 1, { ax: 0.48, flash: e.flash > 0, rot: e.state === 'swing' ? -0.12 : e.state === 'windup' ? 0.08 : 0 })) { g.restore(); if (!e.alive) g.restore(); return; }
    // Legs.
  ...
  function drawTouchControls(g) {
    if ((COARSE_POINTER || input.lastInputWasTouch) && game.state === 'playing' && !game.paused && !game.relicsOpen) {
      const b = TOUCH_PAUSE;
      g.save();
      g.fillStyle = 'rgba(5,3,6,0.78)';
      roundedRectPath(g, b.x, b.y, b.w, b.h, 12); g.fill();
      g.strokeStyle = 'rgba(182,88,107,0.64)'; g.lineWidth = 2; g.stroke();
      g.fillStyle = '#ead8d9';
      g.fillRect(b.x + 27, b.y + 23, 8, 34); g.fillRect(b.x + 45, b.y + 23, 8, 34);
      g.restore();
    }
    if (game.paused || game.relicsOpen) return;
  ...
  function drawMenuButton(g, r, label) {
    const hover = !input.lastInputWasTouch && containsPoint(r, input.mouse);
    g.save();
    g.fillStyle = hover ? 'rgba(108,20,41,0.88)' : 'rgba(35,8,19,0.92)';
    roundedRectPath(g, r.x, r.y, r.w, r.h, 8); g.fill();
    g.strokeStyle = hover ? '#f5768b' : '#823248'; g.lineWidth = 2; g.stroke();
    g.fillStyle = '#ead8d9'; g.textAlign = 'center'; g.font = '700 22px Georgia,serif';
    g.fillText(label, r.x + r.w / 2, r.y + r.h / 2 + 8);
    g.restore();
  }
  ...
  function drawTitle(g) {
    ...
    g.fillText(hasSave()
      ? ((COARSE_POINTER || input.lastInputWasTouch) ? 'TOUCH TO CONTINUE' : 'CLICK TO CONTINUE    •    N FOR A NEW DESCENT')
      : ((COARSE_POINTER || input.lastInputWasTouch) ? 'TOUCH TO DESCEND' : 'CLICK OR PRESS ANY KEY'), W/2, 642);
    g.fillStyle='rgba(220,198,201,.69)';g.font='17px Georgia,serif';g.fillText(COARSE_POINTER || input.lastInputWasTouch
    ...
    if (hasSave()) drawMenuButton(g, TITLE_NEW, 'NEW DESCENT');
    g.restore();
  }
  ...
  function drawPause(g) {
    ...
    g.font = '18px Georgia, serif';
    for (const button of PAUSE_ACTIONS) {
      const labels = { resume: 'RESUME', relics: 'RELIQUARY', checkpoint: 'CHECKPOINT', mute: audio.muted ? 'SOUND: OFF' : 'SOUND: ON', fullscreen: document.fullscreenElement ? 'EXIT FULLSCREEN' : 'FULLSCREEN' };
      drawMenuButton(g, button, labels[button.key]);
    }
    g.fillText(input.lastInputWasTouch
      ? 'TAP RESUME TO RETURN     •     SWIPE DOWN DROPS A LEDGE'
  ...
  function render() {
    ...
    if (!game.paused && !game.relicsOpen) {
      drawHUD(ctx);
  ...
  function frame(now) {
    ...
    let stepped = 0;
    if (game.hitStop > 0 && game.state !== 'title' && !game.paused && !game.relicsOpen) {
      game.hitStop -= realDt;
  ...
  window.__HELLSPINDLE__ = {
    assets() {
      const images = [...bgImages, ...Object.values(sprites), ...Object.values(anims).flat()];
      return {
        total: images.length,
        ready: images.filter(i => i.complete && i.naturalWidth > 0 && i.naturalHeight > 0).length,
        failed: images.filter(i => i.complete && !i.naturalWidth).length
      };
    },
    snapshot() {
      const boss = enemies.find(e => e.type === 'boss');
    ...
    openRelics(open = true) { setRelicsOpen(!!open); },
