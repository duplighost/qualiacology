...
export class HUD {
  ...
  _build() {
    this.root.innerHTML = `
      ...
      <div id="hud-bottom-right">
        <div id="weapon-name">RELAY CARBINE</div>
      ...
      <div id="touch-controls">
        <div id="move-hint"><span>MOVE</span></div>
      ...
      <div id="overlay" class="show">
        <div id="overlay-card">
          <h1 id="game-title">RELAY<span>//ECLIPSE</span></h1>
          <p id="game-tag">the relay is under siege</p>
  ...
  setAmmo(a) {
    ...
    this.el.ammoReserve.textContent = a.reserve;
    const out = a.ammo <= 0 && a.reserve <= 0;
    const needsReload = a.ammo <= 0 && a.reserve > 0;
    const low = a.ammo + a.reserve <= Math.max(a.magSize, a.capacity * 0.12);
    this.el.ammoMag.classList.toggle('empty', a.ammo <= 0);
    this.el.lowAmmo.classList.toggle('show', out || (!a.reloading && (needsReload || low)));
    this.el.lowAmmo.textContent = out ? 'NO AMMO' : needsReload ? 'RELOAD' : 'LOW AMMO';
  }

  setReload(state) {
    const progress = clamp01(state?.progress || 0);
    this.el.reloadBar.classList.toggle('show', !!state?.active);
    this.el.reloadFill.style.width = `${progress * 100}%`;
  }
  ...
  showGameOver(stats) {
    this.el.title.innerHTML = 'SIGNAL LOST';
    this.el.tag.textContent = '';
