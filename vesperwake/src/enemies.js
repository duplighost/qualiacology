...
export function drawEnemyEntity(ctx, e, env) {
  ...
  if (!e.dead) {
    ...
    if (e.state === 'telegraph') drawTelegraph(ctx, e, d, env, tp);
    if (e.state === 'attack') drawStrike(ctx, e, d, env);
...
function drawTelegraph(ctx, e, d, env, tp) {
  ...
  const cy = e.y + e.h * 0.5;
  const k = smoothstep(tp);
  const bright = withAlpha('#f4f8f2', 0.24 + k * 0.62);

  ctx.save();
  ctx.lineCap = 'round';

  /* a bracket that closes on the enemy — the universal "it is coming" shape */
  const r = lerp(Math.max(e.w, e.h) * 0.95 + 26, Math.max(e.w, e.h) * 0.52, k);
  ctx.strokeStyle = bright;
  ctx.lineWidth = 1.6 + k * 1.8;
  for (const side of [-1, 1]) {
    ctx.beginPath();
    ctx.arc(cx, cy, r, side > 0 ? -0.85 : Math.PI - 0.85, side > 0 ? 0.85 : Math.PI + 0.85);
    ctx.stroke();
  }
  /* and a tick that fills clockwise so the TIMING is visible, not just the fact */
