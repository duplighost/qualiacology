// LIFTED VERBATIM from Projects/vigil/src/engine/math.js (2026-09-02).
// Duskfall lineage. damp() is the frame-rate-independent smoothing backbone and Spring
// is the underdamped unit the whole feel layer is built from - COMBAT_FEEL gives its
// springs as freq/damping so they drop straight in. Rng.fork(name) is why adding an
// effect never shifts an existing one's stream. Do not re-derive any of this.
...
export class Spring {
  ...
  update(dt) {
    // Substep so the spring stays stable across a clamped 50 ms frame.
    //
    // The original (VIGIL, and DUSKFALL before it) sized this for the STIFFNESS term only:
    // ceil(dt * w / 1.2). Semi-implicit Euler also needs the DAMPING term to stay bounded,
    // 2*z*w*h < 2, and a critically damped spring violates it long before a stiff one does.
    //
    // Measured on this machine 2026-09-02, freq 9 / damping 1.0 (CFG.player.springs.eye):
    // w = 56.5, dt = 1/60 chose exactly ONE substep, the update matrix had an eigenvalue of
    // -1.40, and a unit impulse reached 4.97e6 metres in one second. The eye spring drives
    // the camera height, so that is the camera leaving the county. The player owner hit it,
    // measured it, and routed around it locally; this is the shared fix so gfx and weapons
    // do not each hit it in turn.
    const steps = Math.max(1, Math.ceil(dt * this.w * (1 + 2 * this.z) / 1.2));
    const h = dt / steps;
