...
import {createBossGroundLibrary,buildBossPlaceArt,stepBossPlaceArt} from './boss-site-art.js';
...
export class BossSites {
 ...
 async init(){this.group=new THREE.Group();this.group.name='boss-destinations';this.ctx.scene.add(this.group);const textures=this._sys('places')?.surfaceTextures;this.materials={};
  ...
  this.material=this.materials.stone;this.ownedMaterials=new Set(Object.values(this.materials));this.groundLibrary=createBossGroundLibrary();
 ...
 _build(s){const root=new THREE.Group();root.name='arena-'+s.id;root.position.set(s.x,s.y,s.z);this.group.add(root);const k=new Kit(),glow=new Kit();const collision=this._sys('collision');const chunk='boss-site:'+s.id;
  ...
  // Each place supplies its own continuous ground, architectural cover and
  // perimeter. No generic grey disc or shared ring of stone barricades.
  if(s.shape==='burrow'){
   ...
   for(const side of[-1,1]){box(side*37,-14,2.8,1.4,1.2,[.19,.11,.075]);for(let i=0;i<12;i++)k.tube(.16,.16,.07,8,side*35-i*.5,.17,-14,IRON);}
   ...
   for(let i=0;i<14;i++){k.box(1.6,.19,19,(i-6.5)*1.8,.27,26,TIMBER);collision?.addCollider({kind:'obb',x:s.x+(i-6.5)*1.8,z:s.z+26,hx:.8,hz:9.5,y0:s.y+.17,y1:s.y+.365,tag:'floor',standable:true,authored:true},chunk);}box(0,-34,31,3,5,IRON);sign(root,'BRINE LOCK|NO VESSEL RETURNED INTACT',0,3.5,37);
   ...
   s.orchard=[];for(let i=0;i<16;i++){const a=i*2.4,r=29+(i%3)*3,x=Math.cos(a)*r,z=Math.sin(a)*r;
    ...
    collision?.addCollider({kind:'circle',x:s.x+x,z:s.z+z,r:.74,y0:s.y,y1:s.y+4,tag:'trunk',authored:true},chunk);box(x+1,z,1.2,.85,.65,[.22,.12,.08]);}sign(root,'WIDOW\'S ORCHARD|DO NOT PICK WHAT LOOKS BACK',0,2.8,39);
  ...
  if(s.id==='underkeep'){
   ...
   });s.cryptMaterials=[...local.values(),s.groundMaterial,...(s.atmosphere?.mats||[])].filter(Boolean);
  }
