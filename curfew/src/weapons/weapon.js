...
export class Weapons {
  ...
  constructor(ctx) {
    ...
    this._prev = { fire: false, aim: false, reload: false, melee: false, sprint: false };
    this._in = { fire: false, aim: false, reload: false, melee: false, sprint: false };
  ...
  /** Swap the selected weapon. M0 only ever calls this with 'bolt'. */
  select(id) {
    ...
    // One frozen def object per selection; read everywhere, written nowhere.
    this.def = {
      id,
    ...
    // A parked reload belongs to the gun it was parked from. Swapping weapons throws it
    // away rather than resuming somebody else's beat sheet at somebody else's timestamp.
    this._parked = null;
   ...
   * the verb it buys cannot be performed. With no node owned this is unchanged.
   */
  get wantsSprintCancel() {
    const i = this._input();
    return i.fire || i.aim || this.buffered > 0
      || (!!this.reloading && !this._canParkReload());
  }
  ammoState() {
  ...
  dump() {
    return {
      ...
      reloading: this.reloading && {
        t: this.reloading.t, empty: this.reloading.empty,
  ...
  step(dt) {
    ...
    const wantAds = i.aim && !p.sprinting && !this.melee
    ...
    if (this.meleeBuffered > 0 && !this.melee && !dead) {
    ...
    else if (reloadPressed) this._startReload();
    if (this.reloading) {
      const r = this.reloading;
      ...
      if (r.t >= r.dur) {
        ...
        this.reloading = null;
        this._pulse('reload:end');
      } else if (i.sprint && this._canParkReload() && !this.melee) {
        // HANDS 'Carry': running cancels it and PARKS it. Legal at any point in the
        // timeline — the answer to something arriving does not wait for `cancelopen`.
