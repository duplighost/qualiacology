...
export class BossEncounters {
 ...
 _stalk(k,p,dt){
  ...
  if((k.stateT>(k.phase===1?.9:.6)&&d<desired+4)||k.stateT>(k.phase===1?2.8:2.1)||pace===0)this._beginAttack(k,p);
 }
 ...
 _beginAttack(k,p){
  this._clearHazards(k);k.attack=k.def.attacks[k.cycle++%k.def.attacks.length];k.attackFamily='slam';k.moveSpeed=0;k.state='warning';k.stateT=0;k.weakOpen=false;k.hitCooldown=0;
  ...
  k.chargeDuration=k.chargeStart?1.05:0;
 ...
 _advanceAttack(k,p,dt){
  const warning=k.state==='warning',t=k.stateT;let strikeBeat=false;
  for(const h of k.hazards){
   const local=t-h.delay,waiting=warning||local<0,active=!warning&&local>=0&&local<=h.life;
   if(active&&!h.sounded){h.sounded=true;if(h.delay>.3)strikeBeat=true;}
   const tell=warning?clamp(t/k.windup,0,1):clamp(1+local/Math.max(.01,h.delay),0,1);
   h.material.opacity=waiting?.13+tell*.23:active?.31*(1-clamp(local/h.life,0,1)*.35):.04;
   ...
   for(const m of h.spikes){
    ...
    // A travelling split grows from its source, matching the damage segment.
    if(h.travel){const f=active?clamp(local/h.travel,0,1):1;m.scale.x=f;m.scale.z=f;}
   }
  }
  if(strikeBeat)this._voice(k,'strike',.20);
  ...
  if(k.chargeStart){
   ...
   if(obstacle?.hit){this._clearHazards(k);k.state='recover';k.stateT=-.8;k.weakOpen=true;k.stagger=3.1;this._seat(k);this._voice(k,'strike',.65);return;}
  ...
  if(!p.dead&&k.hitCooldown<=0&&Math.abs(p.pos.y-k.site.y)<12){
   for(const h of k.hazards){
    let inside=hazardContains(h,p.pos.x,p.pos.z,Math.max(0,p.pos.y-h.y),t);if(h.bodyOnly)inside=t<=k.chargeDuration&&Math.abs(p.pos.y-k.pos.y)<4.2&&Math.hypot(p.pos.x-k.pos.x,p.pos.z-k.pos.z)<4.3;
    if(!inside||h.hit)continue;
    // Walls stop projected attacks from their actual source. Ground waves can be jumped.
    let blocked=false;if(h.kind!=='ring'){
     const sx=h.bodyOnly?k.pos.x:h.x,sz=h.bodyOnly?k.pos.z:h.z;
 ...
 damage(enemy,amount,info={}){if(enemy?.anchor){const k=enemy.boss,a=enemy.record;if(a.spent||!k.alive)return {killed:false,hpFrac:0};a.hp-=Math.max(1,amount);this._wake(k);if(a.hp<=0){a.spent=true;enemy.alive=false;k.stagger=6;k.weakOpen=true;k.state='recover';k.stateT=-2;this._clearHazards(k);k.hp-=190;this._voice(k,'strike',.6);this._sys('fx')?.impact?.('flesh',enemy.pos,N,1.4);if(k.hp<=0)this._die(k);return {killed:false,hpFrac:Math.max(0,k.hp/k.def.hp),species:'encounter'};}return {killed:false,hpFrac:a.hp/36,species:'encounter'};}
  ...
  const closed=['bell','furnace','antler'].includes(k.def.shape)?.20:.26;
  const dealt=Math.max(1,amount*(vulnerable?2.65:k.weakOpen?.52:closed));k.hp-=dealt;k.hitFlash=.22;k.recoil=Math.min(1,k.recoil+(vulnerable?.7:.24));if(k.hp<=0){this._die(k);return {killed:true,hpFrac:0,species:'encounter'};}return {killed:false,hpFrac:k.hp/k.def.hp,species:'encounter'};
 }
 ...
 step(dt){if(!this.ctx.ready)return;if(!this.loaded)this._restore();const p=this._sys('player');if(!p?.pos)return;this.time+=dt;this.active=null;this._colliderTimer-=dt;
  for(const k of this.all){k.age+=dt;k.stateT+=dt;k.hitCooldown=Math.max(0,k.hitCooldown-dt);k.hitFlash=Math.max(0,k.hitFlash-dt);k.recoil=Math.max(0,k.recoil-dt*2.5);k.moveSpeed=0;k.stagger=Math.max(0,k.stagger-dt);const d=Math.hypot(p.pos.x-k.home.x,p.pos.z-k.home.z);k.rig.root.visible=d<460;
   ...
   if(k.state==='recover'){
    k.weakOpen=true;const settle=(k.def.shape==='crypt'?2:2.2)-(k.phase-1)*.2;
 ...
 state(){return {active:this.active?.id||null,encounters:this.all.map(k=>({id:k.id,state:k.state,hp:k.hp,maxHp:k.def.hp,phase:k.phase,weakOpen:k.weakOpen,alive:k.alive,position:k.pos.toArray(),hazards:k.hazards.map(h=>({kind:h.kind,x:h.x,z:h.z,r:h.r,width:h.width,damage:h.damage,delay:h.delay,life:h.life,bodyOnly:!!h.bodyOnly})),anchors:k.site.anchors.filter(a=>!a.spent).length}))};}
